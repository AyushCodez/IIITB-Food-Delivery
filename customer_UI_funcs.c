...
int add_money(char* filename, char*current_cust1, char*username, char*password, char*name, char*address, char*phone, double money_in_acc){
	...
	// printf("customer data:%s\n",updated_customer_data);
	// printf("phone:%s\n",phone);
	sprintf(mny,"%.2f",money_in_acc);
	strcat(updated_customer_data,delimiter);
	// printf("customer data:%s\n",updated_customer_data);
	// printf("money:%s\n",mny);
	strcat(updated_customer_data,mny);
	// printf("customer data:%s\n",updated_customer_data);
	strcat(updated_customer_data,delimiter);
	// printf("customer data:%s\n",updated_customer_data);
	strcat(updated_customer_data,new_line);
	// printf("customer data:%s\n",updated_customer_data);
	char linear_data[261];
	FILE * ptr_w=fopen("cust_det2.txt","w");
	FILE * ptr_r=fopen(filename,"r");
	while(fgets(linear_data,261,ptr_r)!=NULL){
		if(strcmp(linear_data,current_cust1)!=0){
			//print linear_data
			// printf("AYUSHHHHHH:%s\n",linear_data);
			fputs(linear_data,ptr_w);
		} 
		else {
			// printf("SARVESHHHHHH:%s\n",updated_customer_data);
			fputs(updated_customer_data,ptr_w);
		}
	}
	fclose(ptr_w);
	fclose(ptr_r);
	remove(filename);
	rename("cust_det2.txt",filename);
	strcpy(current_cust1,updated_customer_data);
	return 1;
}
int profile(char ln[]){
		system("clear");
...
void login_customer()
{
	...
	while(repeat==1){
		repeat=username_pass(pre_msg_lgn,cst_det);
	}
	pro:
	...
	scanf("%s",opt);
		if(strcmp(opt,"<")==0){
			...
			int repeat=0;
			while(repeat==0)
				repeat=add_money(cst_det,current_cust,username,password,name,address,phone,money_in_acc);
			...
			goto pro;
			   ...
			   float price = main_order(c1,"lativ.txt","cust_order.txt","num_of_delivs.txt");
			   update_wallet(price);
			   goto pro;
		}
